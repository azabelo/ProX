#!/usr/bin/env python3
"""
Print the n-th document (1-based) from local C4 data.

- Parquet (e.g. gair-prox/c4-pro under data/*.parquet): sorted shards, global row index;
  all columns are read from the file (often only `text`).
- Official allenai/c4 English shards are *.json.gz (JSON Lines); use --format json.gz
  if you point --data-dir at a folder containing those files.

Environment: C4_DATA_DIR (default: ./data/raw/C4-one-shard/data next to my_Scripts).
"""
...
def _default_data_dir() -> str:
    ...
    return os.path.normpath(os.path.join(here, "..", "data", "raw", "C4-one-shard", "data"))
...
def _get_nth_json_gz(data_dir: str, n_one_based: int) -> tuple[dict, str, int]:
    if n_one_based < 1:
        raise ValueError("n must be >= 1 (1 = first document).")

    paths = _collect_json_gz_paths(data_dir)
    if not paths:
        raise FileNotFoundError(f"No *.json.gz under {data_dir!r}")
...
def main() -> None:
    ...
    parser.add_argument(
        "n",
        type=int,
        help="Document number (1-based) across sorted shards.",
    )
    parser.add_argument(
        "--data-dir",
        default=os.environ.get("C4_DATA_DIR", _default_data_dir()),
        help="Folder with *.parquet shards or nested en/*.json.gz",
    )
    parser.add_argument(
        "--format",
        choices=("auto", "parquet", "json.gz"),
        default="auto",
        help="auto: parquet if any *.parquet exists under data-dir, else json.gz",
    )
    parser.add_argument(
        "--text-chars",
        type=int,
    ...
    fmt = args.format
    if fmt == "auto":
        fmt = "parquet" if _collect_parquet_paths(data_dir) else "json.gz"

    try:
        if fmt == "parquet":
            record, shard, idx_in_shard = _get_nth_parquet(data_dir, args.n)
        else:
            record, shard, idx_in_shard = _get_nth_json_gz(data_dir, args.n)
    ...
    for k, v in sorted(record.items()):
        if k == "text" or k in ("content", "raw_content"):
            continue
