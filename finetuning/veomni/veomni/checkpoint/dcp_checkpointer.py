...
def _load_dcp_into_state_dict_local(state_dict, checkpoint_path: str) -> None:
    ...
    ``torch.distributed.checkpoint.load`` gained ``no_dist`` in newer PyTorch; older builds
    reject that keyword — omit it for compatibility.
    """
    kwargs = {
        "checkpoint_id": checkpoint_path,
        "storage_reader": FileSystemReader(checkpoint_path),
    }
    if "no_dist" in inspect.signature(load).parameters:
        load(state_dict, no_dist=True, **kwargs)
    else:
        load(state_dict, **kwargs)
...
def _process_shard(
    ...
    _load_dcp_into_state_dict_local(state_dict, checkpoint_path)
