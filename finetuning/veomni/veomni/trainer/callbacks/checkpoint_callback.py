...
class HuggingfaceCkptCallback(CheckpointerCallback):
    ...
    def on_train_end(self, state: TrainerState, **kwargs):
        if self.save_hf_weights:
            # Always export HF at train end so `.../global_step_*/hf_ckpt/` exists for vLLM / HuggingFace
            # even if the last on_step_end HF pass was skipped (e.g. max_steps not divisible by hf_save_steps)
            # or a prior step-end export did not run.
