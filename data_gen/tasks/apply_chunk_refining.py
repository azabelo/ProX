...
def main(args):
    ...
    print(
        f"[chunk rank {RANK}/{TOTAL_SPLIT}] startup: loading tokenizer ({args.model_path}) …",
        flush=True,
    )
    tokenizer, tokenizer_slow_fallback = load_auto_tokenizer_fast_fallback(args.model_path)
    if tokenizer_slow_fallback:
        print(
            f"[chunk rank {RANK}/{TOTAL_SPLIT}] fast tokenizer unavailable for this checkpoint; "
            f"using slow tokenizer and vLLM tokenizer_mode=slow.",
            flush=True,
        )
    print(
        f"[chunk rank {RANK}/{TOTAL_SPLIT}] tokenizer ready; data_path={config.data_path!r}",
        flush=True,
    )
    ...
    if args.data_format == "parquet" and paths and data_folder is not None:
        print(
            f"[chunk rank {RANK}/{TOTAL_SPLIT}] counting rows in parquet (metadata scan) …",
            flush=True,
        )
        ...
        print(
            f"[chunk rank {RANK}/{TOTAL_SPLIT}] parquet rows for this rank: {rank_doc_total}",
            flush=True,
        )
    ...
    print(
        f"[chunk rank {RANK}/{TOTAL_SPLIT}] stream_window={stream_window} "
    ...
    if torch.cuda.is_available():
        ...
        print(
            f"[chunk rank {RANK}/{TOTAL_SPLIT}] CUDA mem before vLLM: "
            f"{free_g:.2f} GiB free / {total_g:.2f} GiB total.{extra}",
            flush=True,
        )
    print(
        f"[chunk rank {RANK}/{TOTAL_SPLIT}] loading vLLM ({args.model_path}) — "
    ...
    print(
        f"[chunk rank {RANK}/{TOTAL_SPLIT}] vLLM ready; entering read/generate loop",
        flush=True,
    )
    ...
    def _bump_chunk_doc_progress(docs_done: int) -> None:
        ...
        while docs_done >= _progress_last_milestone + 100:
            ...
            prefix = f"[chunk rank {RANK}/{TOTAL_SPLIT}] finished {_progress_last_milestone} documents"
    ...
    for doc in tqdm(doc_iter, desc="reading documents"):
        ...
        docs_read += 1
        if docs_read % read_heartbeat == 0:
            print(
                f"[chunk rank {RANK}/{TOTAL_SPLIT}] read {docs_read} docs from input; "
        ...
        if len(window) >= stream_window:
            ...
            window = []
            print(
                f"[chunk rank {RANK}/{TOTAL_SPLIT}] finished vLLM batch; "
                f"cumulative refined docs this rank: {stats['documents']}",
                flush=True,
            )
